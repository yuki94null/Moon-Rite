...
data modify storage yrh.013:global tmp.path set value "yrh.013:item/exception"
...
execute if predicate {condition:"entity_properties",entity:"this",predicate:{"minecraft:type_specific/player":{input:{sneak:false}}}} \
    as @e[tag=yrh013.function.looking_entity,nbt={Brain:{}}] unless score @s yrh013.is_target matches 1.. run \
...
execute if predicate {condition:"entity_properties",entity:"this",predicate:{"minecraft:type_specific/player":{input:{sneak:true}}}} \
    as @e[tag=yrh013.function.looking_entity,nbt={Brain:{}}] if score @s yrh013.is_target matches 1.. run \
